...
Applies GUJSA formatting rules:
  - 11pt Times New Roman, justified, single spacing
  - 8pt space before / 12pt after paragraphs
  - Abstract 9pt
  - Section headings ALL CAPS, bold, 11pt
  - Figure / table captions centered, 11pt italic
...
def body(doc, text, bold=False, italic=False, align=WD_ALIGN_PARAGRAPH.JUSTIFY,
         before=8, after=12, size=BODY_SIZE):
    p = doc.add_paragraph()
    p.alignment = align
...
def heading(doc, text, level=1):
    """Section heading — ALL CAPS bold, no TOC style."""
    p = doc.add_paragraph()
...
def keywords_line(doc, label, text, size=ABS_SIZE):
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
    _set_spacing(p, before=2, after=8)
    rl = p.add_run(label + " ")
...
def build():
    ...
    # Page margins (2cm all around — journal typical)
    ...
    _set_spacing(p2, before=2, after=4)
    r2 = p2.add_run(
    ...
    _set_spacing(pa, before=6, after=2)
    ...
    _set_spacing(pa2, before=0, after=2)
    ra2 = pa2.add_run(
    ...
    abstract_block(doc, "Öz:", (
        ...
        "kullanmaktadır. LightGBM en yüksek ROC-AUC değerine (0,9549) ulaşmış, Derin YSA 0,9537 ile "
        "derin öğrenme modelleri arasında öne çıkmıştır. Sistem Hugging Face Spaces üzerinde gerçek "
        "zamanlı analiz sunan bir web uygulaması olarak dağıtılmıştır."
    ))
    ...
    body(doc, (
        "The proliferation of AI-generated content has extended beyond text and imagery into the "
        "musical domain. Platforms such as Suno (v3, v3.5, v4), Udio, Meta's MusicGen, and numerous "
        "diffusion-based audio synthesis systems (AudioLDM2, Stable Audio, Riffusion) now produce music "
        "that is perceptually indistinguishable from human-composed recordings for many listeners "
        "(Copet et al., 2023; Liu et al., 2023). This development raises substantial questions regarding "
        "copyright attribution, artistic authenticity, and the integrity of music streaming platforms."
    ))
    body(doc, (
        ...
        "challenges absent in speech: harmonic complexity, polyphonic structure, rhythmic patterns, and "
        "the absence of speaker identity cues that aid speech-based detectors."
    ))
    body(doc, (
        "Recent surveys (Liu et al., 2024; Yi et al., 2023) characterize the field as nascent, with most "
        "published work relying on either simple spectrogram-based classifiers or direct transfer of "
        "speech deepfake detection architectures without domain adaptation. Bhatt et al. (2025) identify "
        "that AI-music detectors trained on single generators generalize poorly to unseen synthesis "
    ...
    for item in [
        ...
        "1D-CNN, Residual MLP, Attention MLP), trained with Youden's J threshold optimization and "
    ...
    body(doc, (
        "The system is publicly available at https://huggingface.co/spaces/Rtur2003/AURIS."
    ))
    ...
    body(doc, (
        ...
        "features combined with lightweight classifiers could achieve high detection rates on known "
        "vocoders but degraded substantially on unseen architectures. The ADD 2022 challenge "
        "(Yi et al., 2022) formalized the problem with three tracks covering low-quality fakes, "
        "partially fake audio, and adversarial conditions. Yi et al. (2023) provide a comprehensive "
        "survey of the field, cataloguing feature engineering approaches (MFCC, LFCC, CQT, "
        "mel-spectrogram) and deep learning classifiers across seventeen datasets."
    ))
    ...
    body(doc, (
        ...
        "variants have demonstrated strong performance across audio classification tasks. "
        "Martín-Doñas & Álvarez (2022) applied wav2vec2 directly to the ADD 2022 deepfake detection "
        "challenge, achieving competitive results without task-specific feature engineering. "
        "CLAP (Elizalde et al., 2023) and its LAION variant (Wu et al., 2023) extend contrastive "
        "pre-training to joint audio-text embedding spaces, enabling zero-shot audio classification."
    ))
    ...
    body(doc, (
        ...
        "Kostrzewa et al. (2022) demonstrate that wide ensembles of neural networks reduce variance "
        "and improve generalization in music genre classification. Gradient boosting methods — "
        "particularly XGBoost and LightGBM — show strong performance on handcrafted audio feature "
        "vectors (Gan et al., 2024; Liu et al., 2022). Gourisaria et al. (2024) establish that "
        "combining MFCC and STFT features consistently outperforms either feature set alone, "
        "motivating AURIS's hybrid 47-feature representation."
    ))
    ...
    body(doc, (
        ...
        "music generation model conditioned on text and melody. AudioLDM (Liu et al., 2023) uses "
        "CLAP embeddings as conditioning signals for diffusion-based generation. Alongside commercial "
        "systems Suno and Udio — which deploy proprietary architectures — these systems constitute "
        "the primary generation pipelines whose output AURIS is trained to detect."
    ))
    ...
    body(doc, (
        ...
        "MusicLDM, Tango) and human music corpora including GTZAN (marsyas/gtzan) and the Free Music "
        "Archive (benjamin-paine/free-music-archive-small). Samples span 20 musical genres. "
        "All audio was resampled to 22,050 Hz. Duration and sample rate metadata were excluded from "
        "feature vectors to prevent data leakage."
    ))
    ...
    for cat in [
        ...
        "bandwidth, rolloff, flatness (mean+std), contrast, regularity.",
        ...
        "Harmonic/Tonal (9): chroma mean std, chroma entropy/transition rate, tonnetz std, "
        "harmonic ratio/structure, mel flatness, pitch mean/std.",
    ...
    body(doc, (
        ...
        "(1) StandardScaler is fitted on the training split only; "
        "(2) the optimal threshold is determined via Youden's J statistic "
        "(threshold* = argmax(TPR − FPR) on the fold ROC curve), replacing the naive 0.5 default; "
        "(3) metrics are computed on the validation split at the optimal threshold. "
        "Final reported metrics are macro-averaged across five folds."
    ))

    # ── 4. Results ─────────────────────────────────────────────────────
    heading(doc, "4. Results and Discussion")
    subheading(doc, "4.1. Results")
    body(doc, (
        "Table 1 reports 5-fold cross-validation results for all eleven models sorted by ROC-AUC. "
        "LightGBM achieved the highest ROC-AUC (0.9549). Deep MLP (512-256-128-64) achieved the "
        "highest accuracy (0.8849) and F1 (0.8596) overall, with AUC of 0.9537 — only 0.001 below "
        "LightGBM. The 1D-CNN and Logistic Regression are the weakest performers."
    ))
    ...
    body(doc, (
        "Table 2 reports fold-level AUC stability for DL models. Deep MLP shows the lowest variance "
        "(std=0.0036), while 1D-CNN shows the highest (std=0.0087)."
    ))
    ...
    body(doc, (
        "LightGBM feature importance analysis identifies spectral_flatness_std (0.0619), "
        ...
        "suggesting AI generation systems produce systematically more tonal signals than human recordings "
        "which include room acoustics and natural noise (Figure 7)."
    ))

    body(doc, (
        "Figure references: System pipeline (Figure 1), ROC curves (Figure 2), Confusion matrix "
        "(Figure 3), Model comparison (Figure 4), ML vs DL (Figure 5), CV AUC table (Figure 6), "
        "Feature importance (Figure 7), Score distribution (Figure 8), Calibration curve (Figure 9), "
        "Precision-recall curve (Figure 10)."
    ))

    subheading(doc, "4.2. Discussion")
    body(doc, (
        "The close performance between LightGBM (AUC=0.9549) and Deep MLP (AUC=0.9537) demonstrates "
        "that the 47 handcrafted acoustic features encode nearly all discriminative information "
        "available from the audio. Both model families operate on the same feature vectors, so the "
        "feature engineering pipeline is the primary determinant of performance. The 1D-CNN's "
        "underperformance (AUC=0.8442) is explained by the architectural mismatch: a 47-dimensional "
        "flat feature vector lacks the local temporal correlations that 1D convolutions are designed "
        "to exploit."
    ))
    body(doc, (
        "Spectral flatness dominance in feature importance is interpretable: AI generation systems "
        "optimize perceptual quality metrics favoring tonal richness, producing lower spectral flatness "
        "(more tonal) than human recordings. This is consistent with observations in audio deepfake "
        "detection literature where synthesized audio tends to be spectrally cleaner (Yi et al., 2023)."
    ))
    body(doc, (
        "The AIME dataset exposes the model to 12 different AI architectures. The high AUC achieved "
        "despite this diversity suggests the 47-feature representation captures generator-agnostic "
        "artifacts rather than generator-specific fingerprints, which is critical for deployment against "
        "unseen systems (Bhatt et al., 2025)."
    ))
    body(doc, (
        ...
        "(3) Genre-stratified evaluation would provide more rigorous generalization assessment. "
        "(4) Adversarial robustness (MP3 compression, pitch shifting) has not been evaluated."
    ))
    ...
    body(doc, (
        ...
        "architectures. LightGBM achieved the highest ROC-AUC of 0.9549, closely followed by Deep MLP "
        "at 0.9537. Spectral flatness is the single most discriminating feature. Youden's J threshold "
        "optimization consistently outperforms the naive 0.5 threshold under class imbalance. The "
        "system is publicly deployed on Hugging Face Spaces for real-time detection."
    ))
    body(doc, (
        "Future work will focus on: (1) formal cross-generator held-out evaluation, "
        "(2) wav2vec2 5-fold CV integration, (3) adversarial robustness testing, and "
    ...
    refs = [
        ...
        "Bhatt, A., Rajan, A., et al. (2025). AI-generated music detection and its challenges. "
        ...
        "A pathway and overview. arXiv preprint. https://doi.org/10.48550/arXiv.2412.00571",

        "Liu, Y., Yin, Y., Zhu, Q., & Cui, W. (2022). Musical instrument recognition by XGBoost "
        "combining feature fusion. arXiv preprint. https://doi.org/10.48550/arXiv.2206.00901",
        ...
        "detection: A survey. arXiv preprint. https://doi.org/10.48550/arXiv.2308.14970",
    ]
